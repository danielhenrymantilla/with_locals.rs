...
include!("wrap_statements_inside_closure_body.rs");
    ...
    impl VisitMut for ReplaceLetBindingsWithCbCalls<'_> {
        ...
        )
        {
            ...
            if let Some(i) = with_idx {
                let mut stmts_after_with_let = block.stmts.split_off(i + 1);
                let mut let_assign =
                ...
                *func = format_ident!("with_{}", func);
                ...
                let closure_body =
                    match wrap_statements_inside_closure_body(
